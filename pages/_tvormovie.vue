<template>
  ...
  <main
    v-else
    class="bg-black relative text-white pb-12 px-6 overflow-hidden sm:pb-18"
  >
    <!-- the back arrow -->
    ...
    <h1
      class="text-center mb-10 text-2xl md:text-4xl font-bold tracking-widest md:mb-20"
    >
      {{ title }}
    </h1>
    ...
    <div class="sm:flex sm:justify-between md:justify-around sm:mb-28">
      <div class="self-center">
        <img
          ...
          class="mx-auto"
          :src="`https://image.tmdb.org/t/p/w500/${tvANDmovie.backdrop_path}`"
          :alt="`tvshow image ${title}`"
        />
      </div>

      <!-- the story -->
      <div>
        <h1 class="text-2xl mt-10 underline tracking-wide sm:ml-4 sm:text-4xl">
          STORY :
        </h1>
        <p
          style="max-width: 440px"
          class="text-xl sm:text-2xl mx-auto ml-4 my-8 mb-7 tracking-wide"
        >
          {{ tvANDmovie.overview }}
        </p>
      </div>
    ...
    <div class="md:flex md:justify-evenly items-center">
      <div class="flex justify-center">
        <img
          ...
          :src="`https://image.tmdb.org/t/p/w500/${tvANDmovie.poster_path}`"
          :alt="`image of ${title}`"
        />
      </div>
      ...
      <div class="sm:flex sm:justify-center">
        ...
        >
          ...
          <div class="flex justify-between">
            <p>Vote average :</p>
            <p>
              {{ tvANDmovie.vote_average }}
              <IconsStar class="inline-block h-10 w-10 text-yellow-300" />
            </p>
          </div>
          <div class="flex justify-between">
            <p>Status :</p>
...
export default {
  ...
  async fetch() {
    if (this.type == "tvshow")
      var tvshowORmovieData = axios.get(
        `https://api.themoviedb.org/3/tv/${this.id}?api_key=c695182479fa9880b1a52cd4525a0caf`
      );
    else
      var tvshowORmovieData = axios.get(
        `https://api.themoviedb.org/3/movie/${this.id}?api_key=c695182479fa9880b1a52cd4525a0caf`
      );
